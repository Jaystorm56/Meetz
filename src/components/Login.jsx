  ...
  return (
    <div className="w-full max-w-md mx-auto flex flex-col items-center justify-evenly h-[600px]">
      ...
      <TextField
        ...
        fullWidth
        sx={{ mt: 4, borderRadius: 2, '& .MuiOutlinedInput-root': { borderRadius: 2,fontSize: 25, backgroundColor: '#f4f5f9' } }}
      />
      <TextField
        label="Password"
        InputLabelProps={{ sx: { fontSize: '20px' } }}
        ...
        fullWidth
        sx={{ mb: 2, borderRadius: 2, '& .MuiOutlinedInput-root': { borderRadius: 2,fontSize: 25, backgroundColor: '#f4f5f9' } }}
      />
      <Button onClick={handleLogin} sx={{ backgroundColor: '#6D53F4', fontSize: '20px', fontWeight: 500, color: '#FFFFFF', '&:hover': { backgroundColor: '#5C45D3' }, mb: 3, borderRadius: 50, px: 4, py: 1.5 }} fullWidth>Sign In</Button>
      <Button onClick={() => setActiveTab('Signup')} sx={{ color: '#6D53F4', fontSize: '18px', fontWeight: 500 }}>Need an account? Sign up</Button>
      <Button onClick={() => setActiveTab('Forgot')} sx={{ color: '#6D53F4', fontSize: '18px', fontWeight: 500 }}>Forgot Password?</Button>
    </div>
  );
};
